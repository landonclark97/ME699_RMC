...
def parse_args():
    ...
    parser.add_argument('-m', '--hid1_mult', type=int, help='Size of first hidden layer for value and policy NNs'
                             '(integer multiplier of observation dimension)', default=20)
...
def train_fn(num_episodes, gamma, lam, kl_targ, batch_size, hid1_mult, policy_logvar, clipping_range):
    train_fn = Trainer(env, batch_size, gamma, lam, kl_targ, num_episodes, hid1_mult, policy_logvar, clipping_range)
    train_fn.run()
