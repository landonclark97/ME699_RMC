...
s = [0, 0, 0]
g = [0, -math.pi/2, math.pi/4, 0.0, 0.0, 0.0]
...
def reset():
        ...
        # g = [random.uniform(-math.pi,math.pi), random.uniform(-math.pi,math.pi), random.uniform(-math.pi,math.pi)]
        s = [0, 0, 0]
        g = [0.0, -math.pi/2, math.pi/4, 0.0, 0.0, 0.0]
...
def reward(y, y_hat):
        reward_offset = 0
        p_summ = 0.25*np.sum(np.abs(y[0:3]-y_hat[0:3]))
        v_summ = 1.75*np.sum(np.abs(y[3:6]-y_hat[3:6]))
        summ = (p_summ + v_summ)/2.0
        if summ < 2.0:
                reward_offset = (2.0-summ)
        if summ == 0.0:
                summ = 0.001
        return (1.0/summ)+reward_offset
